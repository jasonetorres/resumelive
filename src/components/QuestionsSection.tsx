...
export function QuestionsSection({ currentTarget }: QuestionsSectionProps) {
  ...
  const toggleAnswered = async (questionId: string, isAnswered: boolean) => {
    ...
    if (!error) {
      setQuestions(prev =>
        prev.map(q =>
          q.id === questionId ? { ...q, is_answered: !isAnswered } : q
        )
      );
    }
  };
            ...
            <Card key={question.id} className={`p-3 ${question.is_answered ? 'bg-green-50 border-green-200' : ''}`}>
              <div className="space-y-2">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Button
                    size="sm"
                    variant={question.is_answered ? "default" : "outline"}
